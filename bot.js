...
const {Message, StringSession, Image, Video} = require('./TEABOT/');
const { DataTypes } = require('sequelize');
const { GreetingsDB, getMessage } = require("./plugins/sql/greetings");
const got = require('got');

// Sql
const TeaBotDB = config.DATABASE.define('TeaBot', {
    info: {
...
async function TeaBot () {
    ...
    var StrSes_Db = await TeaBotDB.findAll({
        where: {
          info: 'StringSession'
        }
    });
    ...
    conn.on ('credentials-updated', async () => {
        ...
        if (StrSes_Db.length < 1) {
            await TeaBotDB.create({ info: "StringSession", value: Session.createStringSession(authInfo) });
        } else {
